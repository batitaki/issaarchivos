...
const getMediaByProduct = async (productId) => {
  try {
    const response = await fetch(
      `${import.meta.env.VITE_REACT_APP_API_ENDPOINT}/media/byProduct/${productId}`
    );
    const data = await response.json();
    return data;
  } catch (error) {
...
const getColors = async () => {
  try {
    const response = await fetch(`${import.meta.env.VITE_REACT_APP_API_ENDPOINT}/color/colors`);
    if (!response.ok) {
...
const createColor = async (colorName) => {
  try {
    const response = await fetch(`${import.meta.env.VITE_REACT_APP_API_ENDPOINT}/color/color`, {
      method: 'POST',
      headers: {
